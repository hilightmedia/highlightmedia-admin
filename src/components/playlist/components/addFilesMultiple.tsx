...
export default function BulkAddFilesToPlaylistsDialog({
  open,
  onClose,
  playlistIds,
  onAdded,
    ...
    },
    {
      ...
      render: (item: FileRow) => (
        <div className="flex flex-col">
          <span className="font-medium text-gray-900 truncate max-w-[240px]" title={item.name}>
            {item.name}
          </span>
          <span className="text-xs text-gray-500">
            {item.fileType?.includes("/") ? item.fileType.split("/")[0] : item.fileType || "N/A"}
          </span>
        </div>
      ),
    },
      ...
      <DialogContent className="max-w-[860px] w-[92%] rounded-xl p-6 gap-4 bg-white">
        <DialogHeader className="flex flex-col gap-2">
          <DialogTitle className="text-left">
            {step === 1 ? "Select Folder" : "Select Files"}
          </DialogTitle>
          <DialogDescription className="font-normal">
            {step === 1
              ? "Choose a folder to browse and select files."
              : `Select files to add to ${playlistIds.length} playlist${playlistIds.length === 1 ? "" : "s"}.`}
          </DialogDescription>
        ...
        {step === 1 ? (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button type="button" disabled className="rounded-lg opacity-60 cursor-not-allowed">
                Next
              </Button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm text-gray-600">
                Selected: <span className="font-medium text-gray-900">{checkedFileIds.length}</span>
              </div>
